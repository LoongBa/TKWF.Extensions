...
{
    /// <summary>
    /// 审计日志分析服务实现（internal sealed，V0.3.0）——经 <see cref="AuditLogEntityDataService"/>（SG1 DataService）
    /// 委托聚合/清理。
    /// <para>异常静默处理：聚合/清理失败时记录 Warning 日志并返回空结果/0（对齐 <see cref="AuditLogQueryService"/>
    ...
    internal sealed class AuditLogAnalyticsService : IAuditLogAnalyticsService
    {
        ...
        /// <inheritdoc />
        public async Task<IReadOnlyList<AuditLogDimensionCount>> GetTopServicesAsync(
            int topN = DefaultTopN, TimeSpan? window = null, CancellationToken ct = default)
        {
            ...
            {
                _logger.LogWarning(ex, "审计日志服务聚合失败: GetTopServicesAsync");
                return Array.Empty<AuditLogDimensionCount>();
            }
        }

        /// <inheritdoc />
        public async Task<IReadOnlyList<AuditLogDimensionCount>> GetTopUsersAsync(
            int topN = DefaultTopN, TimeSpan? window = null, CancellationToken ct = default)
        {
            try
            {
            ...
            {
                _logger.LogWarning(ex, "审计日志用户聚合失败: GetTopUsersAsync");
                return Array.Empty<AuditLogDimensionCount>();
            }
        }

        /// <inheritdoc />
        public async Task<AuditLogStats> GetStatsAsync(TimeSpan? window = null, CancellationToken ct = default)
        {
            try
            {
                var (from, to) = ResolveWindow(window);
            ...
            {
                _logger.LogWarning(ex, "审计日志统计失败: GetStatsAsync");
        ...
        public async Task<int> CleanupExpiredAsync(CancellationToken ct = default)
        {
            ...
            {
                _logger.LogWarning(ex, "审计日志保留清理失败: CleanupExpiredAsync");
                return 0;
            }
        }
