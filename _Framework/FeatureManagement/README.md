...
> TKWF 扩展：**功能管理（特性开关）**——编译期 Contributor 定义声明 + 分层值（Global/Tenant/Role/User）+ `IFeatureChecker` 实现（接入框架 `[RequireFeature]` 过滤器）+ 管理 API。
...
| 版本 | v0.1.0（独立起点） |
| 依赖 | `TKWF.Domain`（含框架 `IFeatureChecker`/`RequireFeature`/`AddFeatureCheck` 基座）+ SG1（含 `[FeatureContributor]` 收集，V4.9.114） |
...
IFeatureManager / FeatureManager（分层解析 + 缓存 + Global 唯一性 + 管理写路径） # 门面
...
- **SG1 收集（C1）**：主框架 V4.9.114 新增 `[FeatureContributor]` 编译期收集（`ProjectMetaContextBase.FeatureContributors`）——业务模块标注 + `Define` 声明定义，宿主启动收集。
- **用户契约（C2）**：`IFeatureManager` 接收 `IDomainUser`（租户/认证/角色成员在其上）——不降级 `IUserInfo`；`FeatureChecker` 解析 ambient 用户（`DomainUserContext.CurrentAopUser as IDomainUser`）。
- **管理 API（C3）**：`FeatureManagementApiService`（`[GenerateController]`）写路径（Set/Delete）统一委托 `IFeatureManager`——缓存失效 + Global 唯一性在门面处理；`FeatureValueEntityDataService` 仅内部存储（不标 `[GenerateController]`，裸 CRUD 禁止直通控制器）。
...
### 分层值解析（`IFeatureManager`）

| 层 | ProviderName | ProviderKey | 优先级 |
|----|-------------|-------------|:------:|
| User | `"User"` | UserId | 1（最优先） |
| Role | `"Role"` | 角色名（遍历序首个命中） | 2 |
| Tenant | `"Tenant"` | TenantId | 3 |
| Global | `"Global"` | null | 4 |
| 默认 | — | — | 5（`FeatureDefinition.DefaultValue`，无则调用方参数） |

- 匿名（`!IsAuthenticated`）直查 Global → 默认。
- 缓存：`IMemoryCache` + `NotFoundSentinel` 负缓存；写后 Global 层显式失效（User/Role/Tenant 层 key 含动态 ProviderKey，依赖 TTL 收敛）。
...
| 键 | 默认 | 说明 |
|----|------|------|
| `CacheExpirationSeconds` | `300` | 值缓存 TTL（进程内 IMemoryCache） |
...
- **多实例缓存边界（C5）**：`IMemoryCache` 进程内——SetValue 仅失效本实例，跨实例至多 `CacheExpirationSeconds`（默认 300s）TTL 收敛；即时全局生效建议缩短 TTL 或等 v0.2.0 Feature 变更事件。
- **红线合规**：Store/Manager 无 `IFreeSql`/`IEntityDAC`（全经 DataService 委托）；物理缓存经 `IMemoryCache` 注入。
- **管理写路径**：仅 `IFeatureManager.SetValueAsync/DeleteValueAsync` 为写入口（失效 + 唯一性）；`FeatureValueEntityDataService` 直写绕过缓存（设计上禁止直通控制器）。
