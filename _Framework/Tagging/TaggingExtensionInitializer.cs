...
/// <para>V0.4.0：Options 消费（配置节默认规则 + Store 自动加载）经 <c>InitializeAsync(IServiceProvider sp)</c>
/// 参数获 DI 容器（V4.10.25 ADR78——替代原 IServiceProviderAware setter 注入，对齐 Permissions 先例）。</para>
/// </summary>
    ...
    where TUserInfo : class, IUserInfo, new()
{
    ...
    /// V4.10.25 (ADR78)：sp 经参数传入（替代 IServiceProviderAware setter 注入）。
    /// </summary>
    public override async Task InitializeAsync(IServiceProvider sp)
    {
        ...
        var options = sp2.GetService<IOptions<TaggingOptions>>()?.Value;
        var tagService = sp2.GetService<ITagService>();
